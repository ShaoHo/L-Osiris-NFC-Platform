...
export const QUEUE_NAME = 'ai-generation';
export const JOB_NAME = 'generate-exhibition-day-draft';
...
export function buildDraftContent(prompt, assetMetadata) {
...
export function sanitizeExhibitionHtml(html) {
  return sanitizeHtml(html, SANITIZE_OPTIONS);
}

export function normalizeJobIds(data) {
...
export async function processAiJob(jobId, prisma, log = logger) {
  const aiJob = await prisma.aiGenerationJob.findUnique({
    where: { id: jobId },
  });

  if (!aiJob) {
    log.warn(`AI job not found: ${jobId}.`);
    return;
  }

  if (aiJob.status !== 'PENDING') {
    log.info(`AI job ${jobId} already handled (${aiJob.status}).`);
    return;
  }
  ...
  try {
    ...
    log.info(`Generated draft content for job ${jobId}.`);
  } catch (error) {
    ...
    log.error(`Failed to generate draft for job ${jobId}: ${message}`);
    throw error;
  }
}

export function createWorker({ prisma, connection, log = logger }) {
  const scheduler = new QueueScheduler(QUEUE_NAME, { connection });
  const worker = new Worker(
    QUEUE_NAME,
    async (job) => {
      if (job.name && job.name !== JOB_NAME) {
        log.warn(`Skipping unknown job ${job.name}.`);
        return;
      }
      ...
      for (const jobId of jobIds) {
        try {
          await processAiJob(jobId, prisma, log);
        } catch (error) {
...
export function startWorker({ prisma = createPrismaClient(), connection = getRedisConnection(), log = logger } = {}) {
  const { scheduler, worker } = createWorker({ prisma, connection, log });
  ...
  return { scheduler, worker, prisma };
}
